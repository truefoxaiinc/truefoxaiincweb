...
function CardGrid({ page }: { page: PageData }) {
  ...
  return (
    <Stagger className={`content-card-grid kind-${page.kind}${page.slug === "about" ? " about-card-grid" : page.slug === "why-choose-us" ? " why-card-grid" : page.slug === "careers" ? " careers-card-grid" : page.slug === "products" ? " products-card-grid" : page.slug === "services" ? " services-card-grid" : ""}`}>
    ...
    <div className={`detail-sections ${page.kind === "legal" ? "legal-copy" : ""}`}>
      {page.sections.map((section, index) => (
        <Reveal className="detail-section cinematic-detail" key={section.title} delay={Math.min(index * 0.035, 0.16)}>
          ...
          <div>{section.eyebrow ? <div className="detail-title"><span className="detail-eyebrow">{section.eyebrow}</span><h2>{section.title}</h2></div> : <h2>{section.title}</h2>}{section.text && <p>{section.text}</p>}{section.bullets && <ul>{section.bullets.map(item => <li key={item}>{item}</li>)}</ul>}</div>
        </Reveal>
      ))}
    </div>
  );
...
export default function PageRenderer({ page }: { page: PageData }) {
  ...
  return (
    <main id="main-content">
      ...
      <section className={`inner-hero cinematic-inner-hero${page.slug === "request-quote" ? " request-quote-hero" : page.slug === "about" ? " about-hero" : page.slug === "why-choose-us" ? " why-choose-us-hero" : page.slug === "careers" ? " careers-hero" : page.slug === "products" ? " products-hero" : page.slug === "services" ? " services-hero" : page.slug === "book-demo" ? " book-demo-hero" : ""}`}>
        <InnerHeroMedia />
        <div className="shell inner-hero-content">
          ...
          <Reveal className="inner-console" delay={0.12}>
            {page.slug === "request-quote" ? (
              <>
                <div><span>AT A GLANCE</span><b>06</b></div>
                <p>A PRACTICAL FIRST STEP<br />TOWARD THE RIGHT SYSTEM.</p>
        ...
        {page.slug !== "request-quote" && page.slug !== "about" && page.slug !== "why-choose-us" && page.slug !== "careers" && page.slug !== "products" && page.slug !== "services" && page.slug !== "book-demo" && <div className="inner-hero-index">TFX / {page.eyebrow}</div>}
      </section>
      ...
      {page.slug !== "request-quote" && page.slug !== "why-choose-us" && page.slug !== "careers" && page.slug !== "products" && page.slug !== "services" && page.slug !== "book-demo" && (
      ...
      <section className={`section page-content${page.slug === "request-quote" ? " request-quote-content" : page.slug === "about" ? " about-content" : page.slug === "why-choose-us" ? " why-choose-us-content" : page.slug === "careers" ? " careers-content" : page.slug === "products" ? " products-content" : page.slug === "services" ? " services-content" : page.slug === "book-demo" ? " book-demo-content" : ""}`}>
        <div className="shell">
          {(page.kind === "contact" || page.kind === "form") ? (
            <div className="form-layout">
              <Reveal className="form-aside">
                ...
                <h2>{page.kind === "contact" ? "A clear problem is enough to begin." : page.slug === "request-quote" ? <>GIVE US THE CONTEXT.<br />WE’LL HELP SHAPE THE NEXT STEP.</> : page.slug === "book-demo" ? <>HELP US MAKE THE<br />CONVERSATION USEFUL.</> : "Help us prepare a useful response."}</h2>
                <p>{page.slug === "request-quote" ? "You don’t need a finished specification. Start with the problem, why it matters and what you hope will change." : page.slug === "book-demo" ? "Choose the type of session you need and the capability you’re interested in. Add a little context about your workflow or environment, and we’ll tailor the conversation accordingly." : page.description}</p>
                <ContentSections page={page} />
              </Reveal>
              <Reveal delay={0.1}><LeadForm intent={formIntent} /></Reveal>
            </div>
          ) : (
            <>{page.slug === "careers" && <Reveal className="careers-open-heading"><span className="eyebrow">NOW HIRING</span><h2>CURRENT OPEN POSITION</h2></Reveal>}<CardGrid page={page} />{page.slug === "careers" && <Reveal className="careers-contact"><span className="eyebrow">INTERESTED IN JOINING TRUEFOX AI?</span><p>Send your resume to <a href="mailto:info@truefoxaiinc.com">info@truefoxaiinc.com</a></p></Reveal>}<ContentSections page={page} /></>
          )}
        </div>
      </section>

      {page.cta && page.kind !== "contact" && page.kind !== "form" && (
        <section className={`page-cta cinematic-page-cta${page.slug === "why-choose-us" ? " why-choose-us-cta" : page.slug === "products" ? " products-cta" : page.slug === "services" ? " services-cta" : ""}`}><div className="shell"><Reveal><span className="eyebrow">{page.slug === "about" ? "READY WHEN YOU ARE" : page.slug === "why-choose-us" ? "READY TO MOVE FORWARD?" : page.slug === "products" ? "EXPLORE THE PRODUCTS" : page.slug === "services" ? "READY TO DEFINE THE WORK?" : "NEXT SIGNAL"}</span><h2>{page.slug === "about" ? <>BRING US THE QUESTION.<br />LEAVE WITH A WAY FORWARD.</> : page.slug === "why-choose-us" ? <>FROM OPEN QUESTION<br />TO CLEAR NEXT STEP.</> : page.slug === "products" ? <>SEE HOW IT COULD WORK<br />IN YOUR OPERATION.</> : page.slug === "services" ? <>FROM COMPLEX REQUIREMENT<br />TO CLEAR SCOPE.</> : "Turn the question into a practical plan."}</h2><p>{page.slug === "about" ? "Tell us what needs to work better, the environment it must work within and the outcome you’re aiming for. We’ll help identify the most practical next step." : page.slug === "why-choose-us" ? "Tell us what needs to improve, where it must work and what a successful outcome looks like. We’ll help turn that context into a practical way forward." : page.slug === "products" ? "Tell us about your workflow, environment and requirements. We’ll tailor the demonstration around the products and capabilities most relevant to you." : page.slug === "services" ? "Share the workflow, operating environment and outcome you need. We’ll assess the context, identify the right approach and define a practical path forward." : "Share the workflow, environment and desired result. We will help define the next step."}</p><Link className="button button-primary" href={page.cta.href}>{page.slug === "why-choose-us" ? "DISCUSS YOUR PROJECT" : page.slug === "products" ? "BOOK A PRODUCT DEMO" : page.slug === "services" ? "REQUEST A SCOPING CONVERSATION" : page.cta.label}<ArrowUpRight /></Link></Reveal></div></section>
      )}
    </main>
  );
}
